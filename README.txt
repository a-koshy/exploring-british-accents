...
This folder contains the data, R code and supplemental outputs accompanying the paper 'Exploring British Accents: Modelling the Trap–Bath Split with Functional Data Analysis' by Aranya Koshy and Dr Shahin Tavakoli.


1. DATA
-------
Datasets are stored in /data/. The original NSCV audio files can be obtained from http://wrap.warwick.ac.uk/138368/. The entire BNC audio edition can be accessed at http://www.phon.ox.ac.uk/AudioBNC, and the subset of ``class'' words analysed in this paper are in bnc_sounds.rda. Preprocessed datasets of the NSCV and BNC formants and MFCCs are provided here, and can be recreated from the original audio by running preprocessing.R.
...
2. CODE
-------
R code files are in /code/. This code uses R version 4.0.2. To recreate the models and figures in the paper, run plots.R.
...
- preprocessing.R contains the code that recreates the preprocessed NSCV and BNC datasets starting from the audio files. The ahocoder software (https://aholab.ehu.eus/ahocoder/) needs to be installed separately for extracting MFCCs from sounds or resynthesising sounds from MFCCs.
...
- blast-StoN.wav contains the resynthesised word ``blast'' spoken in a Southern accent and perturbed towards a Northern accent.

- class-NtoS.wav contains the resynthesised word ``class'' spoken in a Northern accent and perturbed towards a Southern accent.


4. AUTHORS
----------
Aranya Koshy (University of Warwick)
Contact address: aranya.koshy@gmail.com

Dr Shahin Tavakoli (University of Warwick)
Contact address: S.Tavakoli@warwick.ac.uk
